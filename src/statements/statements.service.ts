...
export class StatementsService {
  ...
  async findAll(
    ...
    this.logger.debug(
      `Fetching statements from SAP with filters: ${JSON.stringify(query)}`,
    );
  ...
  async getMonthlyConsumption(
    ...
    return {
      ...
      content: await this.buildAccountStatementPdf(
        statement,
        request.year,
        request.month,
      ),
  ...
  private async buildAccountStatementPdf(
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      const maxPaymentDate = this.formatDisplayDate(
        (header.FECPA || "").toString().trim(),
      );
      ...
      doc
        .font("Helvetica-Bold")
        .fontSize(10)
        .text("NRO. DE LA TARJETA", 420, 100, {
          width: 155,
          align: "center",
          lineBreak: false,
        });
      drawCell(
        ...
        155,
        18,
        this.maskCard((header.NUMTR || "").toString().trim()),
        "center",
      );
      ...
      doc.text("CUPO APROBADO", leftSummaryX, summaryY + 4, {
        width: 120,
        align: "left",
      });
      drawCell(
      ...
      doc.font("Helvetica-Bold").fontSize(10);
      doc.text("CUPO UTILIZADO", leftSummaryX, summaryY + summaryRowH + 4, {
        width: 120,
        align: "left",
      });
      drawCell(
      ...
      doc.text(
        "CUPO DISPONIBLE",
        leftSummaryX,
        summaryY + summaryRowH * 2 + 4,
        { width: 120, align: "left" },
      );
      drawCell(
        leftValueX,
        summaryY + summaryRowH * 2,
        85,
        summaryRowH,
        toMoney(availableCredit),
        "right",
        false,
        10,
      );
      ...
      doc.font("Helvetica-Bold").fontSize(10);
      doc.text("FECHA EMISION", rightSummaryX, summaryY + 4, {
        width: 130,
        align: "left",
      });
      drawCell(
      ...
      const colX = [
        ...
        tableX + colW[0] + colW[1] + colW[2] + colW[3],
      ];
      ...
      for (let i = 1; i < colW.length; i += 1) {
        ...
        doc
          .moveTo(x, tableY)
          .lineTo(x, tableY + tableH)
          .stroke();
      }

      doc.font("Helvetica-Bold").fontSize(10);
      doc.text("FECHA", colX[0], tableY + 8, {
        width: colW[0],
        align: "center",
      });
      doc.text("REFERENCIA", colX[1], tableY + 8, {
        width: colW[1],
        align: "center",
      });
      doc.text("DESCRIPCION", colX[2], tableY + 8, {
        width: colW[2],
        align: "center",
      });
      doc.text("VALOR", colX[3], tableY + 8, {
        width: colW[3],
        align: "center",
      });
      doc.text("SALDO DIFERIDO", colX[4], tableY + 8, {
        width: colW[4],
        align: "center",
      });
      ...
      printableDetails.forEach((item: any, index: number) => {
        ...
        const dateValue = isBalanceLine
          ? ""
          : this.formatDisplayDate((item.BLDAT || "").toString().trim());
        const reference = isBalanceLine
          ? ""
          : (item.XBLNR || "").toString().trim();

        doc.text(dateValue, colX[0] + 2, y, {
          width: colW[0] - 4,
          align: "center",
          lineBreak: false,
        });
        doc.text(reference, colX[1] + 2, y, {
          width: colW[1] - 4,
          align: "center",
          lineBreak: false,
        });
        doc.text(description, colX[2] + 4, y, {
          width: colW[2] - 8,
          align: "left",
          lineBreak: false,
        });
        doc.text(toMoney(item.DMBTR), colX[3] + 2, y, {
          width: colW[3] - 8,
          align: "right",
          lineBreak: false,
        });
        doc.text(toMoney(item.SALTR), colX[4] + 2, y, {
          width: colW[4] - 8,
          align: "right",
          lineBreak: false,
        });
      ...
      doc
        .moveTo(page.left, totalsY + 22)
        .lineTo(page.right, totalsY + 22)
        .stroke();
      for (let i = 1; i < 5; i += 1) {
        const x = page.left + totalsColW * i;
        doc
          .moveTo(x, totalsY)
          .lineTo(x, totalsY + totalsH)
          .stroke();
      }
